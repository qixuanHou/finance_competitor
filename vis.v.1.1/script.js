...
d3.csv("2017.csv", function(error, links) {
    ...
    links.forEach(function(link) {
        link.source = nodes[link.source] || 
            (nodes[link.source] = {name: link.source, cik: link.cik, ticker: link.ticker});
        link.target = nodes[link.target] || 
            (nodes[link.target] = {name: link.target, cik: link.ccik, ticker: link.cticker});
    });
    ...
    function dblclick(d) {
        ...
        links.forEach(function(link) {
            if (link.source.ticker == d.ticker) {
                count ++
